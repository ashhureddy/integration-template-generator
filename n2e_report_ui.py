...
def render(app, ciq_wb, mm_objs, controller_objs, edp_index, user_id, date_str,
    ...
    if len(new_nodes) > 1:
        with st.container(border=True):
            ...
            if len(idl_build_type_options) > 1:
                # Same confirmed gap as MCA: a combo can have both a Preferred and Alternate
                # variant (e.g. G3+G3 -> Type C and Type CC) — which physical ports are
                # actually free can't be known from the CIQ alone, so let the engineer pick.
                idl_build_type_final = st.selectbox(
                    "IDL Build Type \u2014 more than one option detected, pick one",
                    ["\u2014 Select \u2014"] + idl_build_type_options, key="n2e_idl_buildtype")
            ...
            # from the same reference table already built for MCA (uploaded reference sheet
            # confirmed identical values for every N2E-supported build type: C/CC/R/T/G/GG/RR/TT).
            ...
            with c1:
                if auto_idle_lines:
                    ...
                    for l in auto_idle_lines:
                        st.caption(l)
                idle = st.text_area("\U0001F4DD IDLe cable details (additional/manual)", key="n2e_idle", height=60)
            ...
            with c2:
                if auto_idly_lines:
                    ...
                    for l in auto_idly_lines:
                        st.caption(l)
                idly = st.text_area("\U0001F4DD IDLy cable details (additional/manual)", key="n2e_idly", height=60)
    ...
    with st.expander("Pending", expanded=True):
        ...
        # Locked alarm ports — confirmed N2E-specific rules, restored per explicit
        # request: the engineer must manually classify which locked port belongs to
        # which scenario (Pre-existing Active Alarms / Loops & Bridge Clips / Power
        # Plant Swap / Not Cleared by FE / Ignore State), since there's no way to
        # automatically infer that from the raw controller-checks data alone.
    ...
    if st.button("Generate N2E Report \u2192", type="primary", key="n2e_generate"):
        ...
        if not N2E_TEMPLATE_PATH.exists():
            ...
            if len(new_nodes) > 1 and idl_build_type_final:
            ...
            _rw_simple(N2E_ROW_MAP["lkf_installation"]["completed"][0], lkf_completed, lkf_c_value)
